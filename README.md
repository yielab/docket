...
- **~8,700 lines** of Bash in the shipped CLI (`lib` + `bin`); ~12,400 including tests and tooling
- **25 commands** (one `cmd_*` per file) + 1 experimental, over **12 helper modules**
- **247 unit tests** + 12 integration scenarios (60 assertions) + 6 specialist-role evals
- **15 specifications** (RFC 2119), validated in CI

```bash
./tests/run-all-tests.sh           # everything
./tests/unit/test-helpers.sh       # 247 unit tests
