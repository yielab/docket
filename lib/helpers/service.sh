...
#
# docket was Linux/systemd-only; service_ctl lets it degrade cleanly elsewhere
...
# Set DOCKET_NO_RESTART=1 to print instead of restarting (useful for testing).
DOCKET_GATEWAY_DIRTY=0

# Which init system manages user services here.
service_manager() {
  if [[ -n "${DOCKET_SERVICE_MANAGER:-}" ]]; then echo "$DOCKET_SERVICE_MANAGER"; return; fi
...
# docket can't drive the unit: is-active reports not-active, others warn and fail —
...
restart_gateway_if_dirty() {
  [[ "$DOCKET_GATEWAY_DIRTY" -eq 0 ]] && return 0
  DOCKET_GATEWAY_DIRTY=0
  restart_gateway
}
