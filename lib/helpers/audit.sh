...
# Disable with DOCKET_NO_AUDIT=1.
...
audit_log() {
  ...
  DOCKET_AUDIT_TS="$ts" DOCKET_AUDIT_USER="$user" \
    python3 - "$logf" "$action" "$detail" <<'PY' 2>/dev/null || true
import json, os, sys
logf, action, detail = sys.argv[1], sys.argv[2], sys.argv[3]
entry = {
    "ts": os.environ.get("DOCKET_AUDIT_TS", ""),
    "user": os.environ.get("DOCKET_AUDIT_USER", ""),
    "pid": os.getppid(),
    "action": action,
    "detail": detail,
}
