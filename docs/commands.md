...
`add`, `approve`, `audit`, `auth`, `context`, `conversations`, `deny`, `edit`, `init`, `list`, `maintain`, `mcp`, `metrics`, `models`, `persona`, `pipeline`, `pod`, `profile`, `roles`, `runs`, `scope`, `serve`, `status`, `trace`, `unwire`, `help` have no alias.
