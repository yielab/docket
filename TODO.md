...
- **Acceptance gate:** [x] completions advertise exactly the live command set (test-enforced); [x] `eval "$(docket completions bash)"` works in a smoke check; [x] suite + goldens green.
- **Size:** M · **Status:** DONE — commit on `pc/ch-8-drift-proof-completions` (not yet merged). Took **(a) Generate**: `cli/_completions.py`'s top-level command table is now built at call time by walking `get_command(app).commands` (Typer's `TyperGroup`, filtering `hidden`) instead of a static string — see the module docstring. This immediately dropped the stale `team`/`tier`/`version` entries and picked up `auth` (which the old literal never had) plus every other live command; descriptions in the zsh table now come from each command's real docstring instead of hand-written prose. Typer commands here are flat (no Click sub-groups), so second-level verb tables (`maintain check|clean|reset`, etc.) aren't derivable from the registry and stay hand-maintained — audited them against each command's actual action parsing while in there and fixed real drift found along the way: `context` was missing `project`, `workflow` was missing `validate`/`plan`, `pod` was missing `list`, `models` was missing `list`/`provider`, and the agent-id catch-all wrongly included `snapshot`/`audit` (neither takes an agent id). Added `auth`'s subcommand table (`status login key setup`), which didn't exist at all before. Regenerated `tests/golden/cases/writers/completions_{bash,zsh}.golden` (diff is exactly the above). Added `tests/python/test_ch8_completions_drift.py`: independently re-derives the live command set from the Typer registry (not via `_completions`'s own helper) and asserts both scripts match it exactly, `team`/tier names/`_json` never appear, the Phase-11 minimum list is present, and a real `bash -c 'eval "$(docket completions bash)"'` subprocess smoke check actually defines `_docket_complete`. Full gate green: `ruff check`, `ruff format --check`, `mypy src`, `pytest` (all green), `tests/golden/run.sh verify-all` (16/16).
