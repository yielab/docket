...
>
> **Wave 34 closed on 2026-09-13** and is archived in
> [docs/cycles-ended/todo-waves.md](docs/cycles-ended/todo-waves.md). It finished the D-36
> function split under a new shrink-only function-span ratchet (four splits, zero behaviour
> change, one deliberate entry left), made the harness cancellation test deterministic under
> load, gave the duplicated provider credential table one owner, and found the fourth
> unwired-machinery instance: `docket gates enable/disable` write a flag that nothing on the
> live path reads, while the spec, README and command help said it changed where an `ask`
> verdict is delivered. The claims are now true; wiring the flag or retiring the commands is an
> open maintainer decision recorded in `specs/functional/security-gates.spec.md`.
>
> **Wave 33 closed the same day** (audit of the Wave 32 closure, repository hygiene, a
> docstring-only comment sweep from 558/112 to 372/40 over-budget docstrings).
>
