...
| CLI Interface | 1.29.0 | Complete | `pod dispatch` and `pipeline run` exit 1 when a task ends `failed`, and pod output prints task ids, roles and reasons literally |
...
| Serve Read API | 2.13.0 | Stable | Concurrent pod provisioning serializes one project while different projects share only the short atomic allocation transition |
