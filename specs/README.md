...
| Session History | 1.1.0 | Implemented and live | `core/session.py`: durable per-session turn history, lossless message round-trip, atomic tool-call/tool-result compaction, isolated summarizer key, recursion guard, and fail-closed behavior; invoked before live task completions by `core/agent_loop.py` (W20-C2) |
