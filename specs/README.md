...
| Session History | 1.2.0 | Implemented and live | `core/session.py`: durable per-session turn history, lossless message round-trip, bounded hierarchical atomic compaction, isolated summarizer key, recursion guard, and whole-operation fail-closed behavior; invoked before live task completions by `core/agent_loop.py` (W20-C2/W20-C2b) |
