...
| Pod Dispatch | 6.5.1 | Complete | Hop execution/history/handoff behavior is live; conversation hops mutate one validated locked registry and retain concurrent updates |
...
| CLI Interface | 1.21.0 | Complete | The root wheel/sdist installs canonical `docket` with artifact-only version/help/init-help and metadata/uninstall contracts |
...
| Runtime Library | 2.0.0 | Implemented (artifact-tested; **not published to any index**) | `docket-runtime` exclusively owns `docket_runtime/`, rebuilds wheel and sdist outside the checkout, coexists safely with the full `docket` distribution, and exposes only the versioned gated-tool facade; private transformed runtime modules remain unsupported internals |
