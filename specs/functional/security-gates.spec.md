...
**Version**: 0.5.0
**Status**: Implemented (on by default for new installs; daemon-enforced — see the approval-seam note below). Docket's own approval store now has a real production producer (ROADMAP Phase 15 G-1); the daemon-gate bridge (G-5) remains not implemented — see the approval-seam note.
...
>
> **The approval seam (updated 2026-07-30 — G-1 shipped, G-5 still open).** There are two
> approval systems in play, and **the daemon-facing half of the seam is still not bridged**:
...
> gate — that bridge is ROADMAP Phase 15 G-5 (daemon-gated spike), not implemented.
>
...
4. Every grant and denial **through docket's approval store MUST** be recorded in the audit
   ...
   `telegram` tag is reserved for the G-5 bridge: today a daemon-side `/approve` writes **no**
   docket audit entry.
