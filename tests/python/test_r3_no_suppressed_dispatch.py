"""R-3 / D-17 guard: no `contextlib.suppress(Exception)` wraps a dispatch call.

Before this card, ``serve.py`` had four places where a pod-dispatch call was
...
This is the "grep-pinned" regression test the card calls for (sibling in
spirit to ``test_ch4_no_subprocess_in_core.py``'s AST-based guard): it scans
...
intentionally left alone — this guard is scoped to the dispatch lane, per the
card ("ban the suppressions ... around dispatch").
"""
...
def test_dispatch_all_pods_no_longer_called_unguarded_in_serve() -> None:
    ...
    losing per-pod granularity. R-3 replaces that with one run record per pod
