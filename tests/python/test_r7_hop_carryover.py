...
hop's share was exceeded. **C-1 retires that mechanism**: ``_hop_message`` no
longer calls ``_hop_carryover_budget``/``_truncate_carryover`` at all — every
prior hop's rendered ``HandoffArtifact`` is now fit to a per-role *token*
budget via ``core/context.py``'s ``compile_artifact``, which sheds the
artifact's own less-valuable fields (``HandoffArtifact.DROP_ORDER``) before
ever truncating ``summary`` itself. See ``tests/python/test_c1_context_
compiler.py`` for that module's own unit tests; this file covers:

  * TestHopCarryoverBudget / TestTruncateCarryover — R-7's two pure helpers,
    still defined in ``core/dispatch.py`` (this card's file-ownership
    carve-out permits editing only ``_hop_message`` in that module — see
    TODO.md's Wave 6 board — so these are no longer called by anything, but
    are left in place, not deleted, for the integrator to remove). Kept here
    as a regression pin on now-dead code, not a claim that it still runs.
...
# ── pure helpers R-7 left behind: no longer called by _hop_message ─────────────
# (core/dispatch.py's `_hop_carryover_budget`/`_truncate_carryover` — retained
# by this card's file-ownership carve-out, which permits editing only
# `_hop_message` in that module; flagged for the integrator to remove.)
