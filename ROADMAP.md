...
| D-19 | Drop the OpenClaw daemon and drive the local llama-server (qwen, :8081) directly? | 2026-07-31, user-requested | **Split the question — a blanket "yes" is not buildable as stated.** `run_turn` is one shell-out (`openclaw agent --agent … --json`) that hides an entire agent runtime: the model loop, tool *execution*, session persistence and compaction. A real session on this host shows 94 tool records (`read` x73, `write` x2). llama-server serves **inference only** — it executes nothing. So replacing the daemon means docket builds a tool-executing agent runtime plus sandboxing, session store, channels and RAG memory, which is the exact "build vs wrap" line §4.5 draws and D-14 reaffirmed. **Adopted instead:** (a) ship a **`local-openai` driver for tool-free turns** — docket's own LLM calls (D-18: distillation, judges, summaries) stop needing the daemon at all; (b) keep the daemon for tool-using pod hops until a replacement is a funded project, not a side effect. See Phase 19 in TODO.md. |
