...
- **The two invariants that replaced the ACL invariant**, both machine-enforced:
  - **Every tool call goes through `core/tools.py`'s dispatcher**, where `pre_input`, `pre_tool_call` and `pre_output` all evaluate. A second execution path is a hole. Enforced by an AST test (`tests/unit/core/test_tools__tool_registry.py::test_only_the_chokepoint_imports_the_handler_module`).
...
| D-37 | When the agent lane is over budget and the test to retire is the only evidence for a sentence in the README, which one goes? | Wave 31 / W31-C2 | **The test structure wins; the claim goes out with the test, in the same change.** A README sentence is an input to the test suite, not a reason to keep an oversized or unmaintainable check inside it. The suite has to be structured, maintainable and correct first; a capability claim that no longer has a test is removed from every user-facing document at the moment its test is, never left standing. Applied at 2026-09-11 to the installed-artifact coverage claim for the two third-party adapter configurations, retired with `test_runtime_openhands_adapter.py`, `test_pydantic_ai_adapter.py` and the harness-script file to bring the lane under 4,000 lines. **What this does not license:** deleting a test to dodge a failure, or weakening a claim the product still makes and still verifies elsewhere. The retirement and the doc edit are one commit, and the commit body names the claim that left. |
